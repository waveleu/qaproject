...
class AuthGroupAccessModel extends BaseModel{
	...
	public function getAllData(){
		...
		foreach ($user_data as $k => $v) {
			...
			foreach ($user_data[$k]['title'] as $subk=>$subv){
				$user_data[$k]['title_list'][$subk]['group_id']=D('AuthGroup')->where(array('title'=>$subv))->getField('id');
				$user_data[$k]['title_list'][$subk]['title']=$subv;
			}
