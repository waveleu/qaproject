...
<div id="content" class="span10">
  ...
  <div class="pageTit page-header">
    <h1><?php if($category_info): ?>编辑<?php else: ?>添加<?php endif; ?>类别 </h1>
    <div class="opt">
      <a class="btn btn-info" href="<?php echo U('admin/category/index');?>">返回类别列表</a>
    </div>
  </div>
  <!-- 标题 end -->
  <!-- 内容区块 start -->
  <div class="formBox">
      <form id="addform" action="<?php echo U('admin/category/save');?>" method="post" enctype="multipart/form-data">
      <input name="id" type="hidden" value="<?php echo ($category_info["id"]); ?>" />
        <!-- 标题 -->
        <div class="control-group">
          <label>类别名称：(<font color="red">最少2个最多50个汉字</font>)</label>
          <input type="text" name="title" value="<?php echo ($category_info["title"]); ?>" <?php if(empty($category_info)): ?>ajaxurl="<?php echo U("admin/category/check_title");?>"<?php endif; ?> datatype="*2-50" nullmsg="请填写类别名称！" errormsg="类别名称至少2最多50个汉字！" />
        ...
        <div class="control-group">
          ...
          <input type="text" name="sort" value="<?php echo ($category_info["sort"]); ?>" datatype="n1-2" nullmsg="请输入排序号！" errormsg="排序号只能为1-2位数字！"/>
          <span class="Validform_checktip"></span>
        </div>
...
$(function(){
  //提交表单合法性验证 
    ...
    callback:function(data){
      if(data.status=="ok"){
          alert(data.info);
		  window.location.href = "<?php echo U('admin/category/index');?>";
      }else{
        alert(data.info);
      }
    }
