...
<div id="content" class="span10">
  ...
  <div class="pageTit page-header">
    <h1><?php if($OS_Version_info): ?>编辑<?php else: ?>添加<?php endif; ?>系统信息 </h1>
    <div class="opt">
      <a class="btn btn-info" href="<?php echo U('admin/category/index');?>">返回系统信息列表</a>
    </div>
  </div>
  <!-- 标题 end -->
  <!-- 内容区块 start -->
  <div class="formBox">
      <form id="addform" action="<?php echo U('admin/OSVersion/save');?>" method="post" enctype="multipart/form-data">
      <input name="OS_Version_ID" type="hidden" value="<?php echo ($OS_Version_info["OS_Version_ID"]); ?>" />
        <!-- 标题 -->
        <div class="control-group">
          <label>系统信息名称：(<font color="red">最少2个最多50个汉字</font>)</label>
          <input type="text" name="Version" value="<?php echo ($OS_Version_info["Version"]); ?>" datatype="*2-50" nullmsg="请填写系统信息名称！" errormsg="系统信息名称至少2最多50个汉字！" />
        ...
        <div class="control-group">
          ...
          <input type="text" name="OS_list" value="<?php echo ($OS_Version_info["OS_list"]); ?>" datatype="*2-50" nullmsg="请输入排序号！" errormsg="排序号只能为1-2位数字！"/>
          <span class="Validform_checktip"></span>
        </div>
