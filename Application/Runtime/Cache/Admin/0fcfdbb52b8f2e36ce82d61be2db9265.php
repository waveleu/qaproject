...
<div id="content" class="span10">
  ...
  <div class="pageTit page-header">
    <h1><?php if($brand_info): ?>编辑<?php else: ?>添加<?php endif; ?>品牌 </h1>
    <div class="opt">
      <a class="btn btn-info" href="<?php echo U('admin/category/index');?>">返回品牌列表</a>
    </div>
  </div>
  <!-- 标题 end -->
  <!-- 内容区块 start -->
  <div class="formBox">
      <form id="addform" action="<?php echo U('admin/brand/save');?>" method="post" enctype="multipart/form-data">
      <input name="id" type="hidden" value="<?php echo ($brand_info["id"]); ?>" />
        <!-- 标题 -->
        <div class="control-group">
          <label>品牌名称：(<font color="red">最少2个最多50个汉字</font>)</label>
          <input type="text" name="title" value="<?php echo ($brand_info["title"]); ?>" datatype="*2-50" nullmsg="请填写品牌名称！" errormsg="品牌名称至少2最多50个汉字！" />
        ...
        <div class="control-group">
          ...
          <input type="text" name="sort" value="<?php echo ($brand_info["sort"]); ?>" datatype="n1-2" nullmsg="请输入排序号！" errormsg="排序号只能为1-2位数字！"/>
          <span class="Validform_checktip"></span>
        </div>
...
$(function(){
  //提交表单合法性验证 
    ...
    callback:function(data){
      if(data.status=="ok"){
          alert(data.info);
		  window.location.href = "<?php echo U('admin/brand/index');?>";
      }else{
        alert(data.info);
      }
    }
